...
|---|---|---|---|
| Retention-checkpoint scheduler (cron/n8n) | Phase 7 | **Yes** | `GET /retention/checkpoints/due` is correct, real application logic; only the external poller is missing. No `n8n`/worker service exists in `docker-compose.yml` yet. Close before any pilot that needs checkpoints to fire without a human manually polling. |
| Role/consent/age-based decision authorization | Phase 6 | **Yes, but time-boxed** | Only two authorization rules exist (escalate `TEACHER_INTERVENTION`, reject low-confidence personalization). No consent/age data model exists. §81 requires this before real minors are on the platform unsupervised — **must close before any pilot involving actual students**, not indefinitely deferrable. |
